"""Self-contained HTML report of an edit — a visual you can open in a browser.

Shows the prompt, a visual kept-vs-cut timeline over the original footage, the
edit plan, edit stats, and a styled caption preview. Doubles as a low-fi mockup
of the eventual app UI. No external assets/CSS — one portable .html file.
"""
...
def _caption_preview(model: EditModel, limit: int = 8) -> str:
    ...
    for ev in cap.events[:limit]:
        words = " ".join(f'<span class="w">{html.escape(w.w)}</span>' for w in ev.words)
        out.append(f'<div class="cap"><span class="t">{ev.start:0.1f}s</span> {words}</div>')
    more = f'<div class="muted">+{len(cap.events) - limit} more…</div>' if len(cap.events) > limit else ""
...
def render_html(prompt: str, model: EditModel, report: dict, original_s: float,
                ffmpeg: str) -> str:
    chips = "".join(
        f'<span class="chip">{html.escape(list(o.keys())[0])}</span>'
    ...
    style = model.captions.style
    hi = model.captions.highlight_color
    return f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>VibeCut · edit report</title>
<style>
  :root {{ --bg:#0f1115; --card:#171a21; --line:#262b36; --txt:#e6e9ef;
           --muted:#8b93a7; --kept:#37d399; --cut:#2a2f3a; --lock:{hi}; --accent:#7aa2ff; }}
  * {{ box-sizing:border-box; }}
  ...
  .seg.locked {{ background:var(--lock); }}
  ...
  .cap .w {{ font-weight:{ 800 if style in ('bold-karaoke','hype') else 500 }; }}
  .cap .w:first-of-type {{ color:{hi}; }}
  ...
  <h1><span class="logo">◆ VibeCut</span> — edit report</h1>
  <div class="prompt">“{html.escape(prompt)}”</div>

  <div class="card">
    <div class="label">Timeline · original footage (green kept, gold locked, dark cut)</div>
  ...
  <div class="card">
    <div class="label">Edit plan</div>
    {chips}
  </div>

  <div class="card">
    <div class="label">Captions preview · style: {html.escape(style)}</div>
    {_caption_preview(model)}
  </div>

  <div class="card">
    <div class="label">Export</div>
    <details><summary>FFmpeg command (hardware encode)</summary>
      <pre>{html.escape(ffmpeg)}</pre>
    </details>
  </div>

  <div class="foot">Generated locally by VibeCut core · no APIs · no generative video.</div>
</div></body></html>
"""
