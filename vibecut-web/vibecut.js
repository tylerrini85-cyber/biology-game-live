...
(function (root) {
  ...
  function edit(analysis, plan, library, prev) {
    ...
    keep.ranges.forEach(function (r) {
      ...
      clips.push({ src_in: r[0], src_out: r[1], timeline_start: off, locked: !!lk, transition_in: "", speed: 1 });
      off += r[1] - r[0];
    });
    ...
    applyClipSpeed(model);  // per-clip speed warp (before overlap compression)
  ...
  // warp timeline + captions/zooms by each clip's per-clip speed
  function applyClipSpeed(model) {
    var clips = model.clips;
    if (!clips.length || clips.every(function (c) { return Math.abs((c.speed || 1) - 1) < 1e-6; })) return;
    var cstarts = clips.map(function (c) { return c.timeline_start; });
    var cdurs = clips.map(function (c) { return c.src_out - c.src_in; });
    var ns = [], o = 0;
    clips.forEach(function (c, i) { ns.push(o); o += cdurs[i] / (c.speed || 1); });
    var warp = function (t) {
      for (var i = 0; i < clips.length; i++) { var cs = cstarts[i], d = cdurs[i];
        if (t < cs + d - 1e-6 || i === clips.length - 1) { var off = Math.min(Math.max(t - cs, 0), d); return ns[i] + off / (clips[i].speed || 1); } }
      return o;
    };
    clips.forEach(function (c, i) { c.timeline_start = ns[i]; });
    (model.broll || []).forEach(function (b) { b.timeline_start = warp(b.timeline_start); });
    model.captions.events.forEach(function (ev) { ev.start = warp(ev.start); ev.end = warp(ev.end); ev.words.forEach(function (w) { w.t = warp(w.t); }); });
    (model.zoomKeyframes || []).forEach(function (k) { k.a = warp(k.a); k.b = warp(k.b); k.c = warp(k.c); });
  }
  ...
  function ffmpeg(model, src, enc) {
    ...
    model.clips.forEach(function (c, i) {
      var ch = "[0:v]trim=start=" + c.src_in.toFixed(3) + ":end=" + c.src_out.toFixed(3) + ",setpts=PTS-STARTPTS";
      if (Math.abs((c.speed || 1) - 1) > 1e-3) ch += ",setpts=PTS/" + (c.speed).toFixed(4);
      parts.push(ch + "[v" + i + "]");
    });
    model.clips.forEach(function (c, i) {
      var ch = "[0:a]atrim=start=" + c.src_in.toFixed(3) + ":end=" + c.src_out.toFixed(3) + ",asetpts=PTS-STARTPTS";
      var s = c.speed || 1;
      if (Math.abs(s - 1) > 1e-3) { var ff = s, ats = []; while (ff > 2) { ats.push("atempo=2.0"); ff /= 2; } while (ff < 0.5) { ats.push("atempo=0.5"); ff *= 2; } ats.push("atempo=" + ff.toFixed(3)); ch += "," + ats.join(","); }
      parts.push(ch + "[a" + i + "]");
    });
    ...
    if (overlap) {
      ...
      var pv = "vn0";
      for (var i = 1; i < n; i++) { var xfn = XFADE[model.clips[i].transition_in] || XFADE[model.transition]; parts.push("[" + pv + "][vn" + i + "]xfade=transition=" + xfn + ":duration=" + D.toFixed(3) + ":offset=" + model.clips[i].timeline_start.toFixed(3) + "[vx" + i + "]"); pv = "vx" + i; }
      vl = pv;
